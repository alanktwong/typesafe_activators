...
object WebSocketActor {
	 ...
	 */
	def actorOf[M](f: RequestHeader => ActorRef => Props)
		(implicit frameFormatter: WebSocket.FrameFormatter[M],
				app: Application,
				messageType: ClassTag[M]): WebSocket[M] =
	{
		WebSocket[M] { request =>
			(enumerator, iteratee) =>
	...
	private object WebSocketsActor {
		 ...
		 */
		case class Connect[M](requestId: Long, enumerator: Enumerator[M], iteratee: Iteratee[M, Unit], createHandler: ActorRef => Props)
					(implicit val messageType: ClassTag[M])
	}
