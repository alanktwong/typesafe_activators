	...
	implicit object TweetUnmarshaller extends Unmarshaller[Tweets] {
				...
				case (Some(JsString(id)), Some(JsString(text)), Some(JsString(createdAt)), Some(JsObject(user))) => {
					user.get("id_str") match {
						case Some(JsString(userId)) => {
							Right(Tweet(id, userId, text, dateFormat.parse(createdAt)))
						}
						case _ => {
							Left(MalformedContent("Bad tweet JSON"))
						}
					}
				}
				case _ => {
					Left(MalformedContent("Bad status JSON"))
				}
			}
		}
		...
		def apply(entity: HttpEntity): Deserialized[Tweets] = {
			...
			json.fields.get("statuses") match {
				case Some(JsArray(statuses)) =>{
					Right(statuses.map(t => mkTweet(t).right.get))
				} 
				case _ => {
					Left(MalformedContent("statuses missing"))
				}
			}
		}
	}
